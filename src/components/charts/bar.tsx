      ...
      {data.map((d, i) => {
        ...
        if (isHorizontal) {
          ...
          if (stacked && stackOffsets) {
            ...
            // LOCAL PATCH (diverges from @bklit/bar-chart): anchor the bar to
            // the zero line rather than the plot's left edge, so negative
            // values extend left instead of producing a negative width. For an
            // all-positive domain `scale(0)` is 0 and this is a no-op.
          ...
          if (stacked && stackOffsets) {
            ...
            // LOCAL PATCH: anchor to the zero line rather than the plot floor,
            // so negative values hang below the baseline instead of producing a
            // negative height. No-op when the domain starts at zero.
            const zeroPos = scale(0) ?? innerHeight;
