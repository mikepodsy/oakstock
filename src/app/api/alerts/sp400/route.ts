...
export interface StoredMomentum extends MomentumStatus {
  name: string | null;
  marketCap: number | null;
}

export interface Sp400Response {
  statuses: StoredMomentum[];
...
// Reads the precomputed large-cap momentum snapshot, biggest market cap first.
export async function GET() {
  ...
  if (error) {
    return NextResponse.json(
      { error: "Failed to read momentum status", detail: error.message },
      { status: 500 }
    );
  }
