...
export interface BarYAxisProps {
  ...
  maxLabels?: number;
  /**
   * LOCAL ADDITION (not in @bklit/bar-chart): label width budget in px before
   * truncation. The registry pins this at 70, which clips long category names
   * even when the left margin has room. Defaults to the old value.
   */
    ...
    >
      {labelsToShow.map((item) => (
        <BarYAxisLabel
          ...
          label={item.label}
          // Fall back to the left margin, minus the label's own right padding,
          // so a caller who budgeted room for long names actually gets it.
          labelMaxWidth={labelMaxWidth ?? Math.max(70, margin.left - 8)}
          y={item.y}
        />
      ))}
    </div>,
