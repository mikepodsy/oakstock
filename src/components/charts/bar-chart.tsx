...
/** Stable anchor for the categorical charts' placeholder time scale (2024-01-01). */
const FAKE_TIME_SCALE_EPOCH = 1_704_067_200_000;
...
const ChartCore = memo(function ChartCore({
  ...
  // LOCAL PATCH (diverges from @bklit/bar-chart): the registry version pins the
  // value domain to [0, max], so negative values collapse onto the baseline and
  // render as zero-height rects. Financial series are routinely signed (net
  // income, free cash flow, COT net positioning), so the extent tracks both ends
  // and the domain keeps zero inside it whenever the data crosses zero.
  // Re-running `shadcn add @bklit/bar-chart` will clobber this — reapply it.
  const valueExtent = useMemo((): [number, number] => {
    let min = 0;
    let max = 0;

    if (stacked) {
      // For stacked bars, sum all values at each data point. Positive and
      // negative contributions accumulate separately so a mixed-sign stack
      // reports the true reach in both directions.
      for (const d of data) {
        let positiveSum = 0;
        let negativeSum = 0;
      ...
      // For grouped bars, find the extreme single value in each direction.
      for (const line of lines) {
        for (const d of data) {
          const value = d[line.dataKey];
          if (typeof value !== "number") {
  ...
  const valueScale = useMemo(() => {
    ...
    return scaleLinear({
      range,
      // Pad only the ends that actually carry data, so an all-positive series
      // still sits flush on a zero baseline exactly as it did before.
      domain: [min < 0 ? min * 1.1 : 0, max > 0 ? max * 1.1 : 0],
      nice: true,
    });
  ...
  const fakeTimeScale = useMemo(() => {
    // LOCAL PATCH: the registry seeds this from `Date.now()`, an impure call
    // during render that yields a different domain on every re-render. A bar
    // chart's x axis is categorical, so this scale exists only to satisfy the
    // shared context type — a fixed epoch keeps it stable and is just as valid.
    const now = FAKE_TIME_SCALE_EPOCH;
