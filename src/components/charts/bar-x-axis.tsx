...
export interface BarXAxisProps {
  ...
  maxLabels?: number;
  /**
   * LOCAL ADDITION (not in @bklit/bar-chart): render the tick text from the
   * category value. Without it the category key doubles as the display label,
   * which forces callers to pre-format `xDataKey` — and any two rows that share
   * a formatted label then collapse into one band.
   */
